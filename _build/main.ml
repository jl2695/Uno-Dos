...
let rec group_hand hand org_hand pile acc idx =
  ...
  | h :: t ->
      ...
      print_string (string_of_int (List.length comp));
      if comp <> [] then
...
let print_group grp =
  ...
  print_string "(";
  print_card first;
  print_string (string_of_int (snd (List.hd grp)));
  print_card second;
  print_string (string_of_int (snd (List.hd (List.rev grp))));
  print_string ")"
...
let rec dos_turns pos st =
  ...
  if not player.ai then (
    ...
    | Draw ->
        ...
        print_endline_centered
          ("Cards left in the deck: " ^ string_of_int deck_length);
        dos_turns next_pos (draw_st st pos deck 1)
    | Place card_index -> (
        match int_of_string card_index with
        | idx ->
            (* Check to see if the card index is valid in the player's
            ...
            if idx <= List.length player.hand - 1 && idx >= 0 then
              ...
              let hand_length = List.length player.hand in
              idx >= hand_length && idx < hand_length + List.length grps
              (******** START DOUBLE MATCH **********)
            then
